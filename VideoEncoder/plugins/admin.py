...
@BotzHub.on(events.NewMessage(from_users=1704388185, pattern="^/broadcast"))
async def _broadcast(event):
    await BotzHub.send_message(
        event.chat_id, "Send the message you want to broadcast!\nSend /cancel to stop."
    )
    async with BotzHub.conversation(1704388185) as conv:
        response = conv.wait_event(events.NewMessage(chats=1704388185))
...
@BotzHub.on(events.NewMessage(from_users=1704388185, pattern="^/stats"))
